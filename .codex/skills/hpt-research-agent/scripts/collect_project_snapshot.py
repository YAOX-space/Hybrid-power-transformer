...
def latest_result_directories(limit: int) -> list[dict[str, Any]]:
    ...
    return [
        {"name": path.name, "modified_epoch": int(path.stat().st_mtime)}
...
def build_snapshot(result_limit: int, log_limit: int) -> dict[str, Any]:
    ...
    return {
        ...
        "latest_result_directories": latest_result_directories(result_limit),
        "canonical_commands": [
