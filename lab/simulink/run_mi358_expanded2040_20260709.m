...
diary(fullfile(logdir, ['simulink_mi358_expanded2040_' run_tag '.log']));
cleanup = onCleanup(@() diary('off')); %#ok<NASGU>

fprintf('=== run_mi358_expanded2040_20260709 %s ===\n', run_tag);
...
for k = 1:chunk_size:numel(sids)
    ...
    chunk_tag = sprintf('mi358_expanded2040_%s_%04d_%04d', run_tag, k, k + numel(chunk) - 1);
    fprintf('=== chunk %d:%d (%d scenarios) %s ===\n', k, k + numel(chunk) - 1, numel(chunk), datestr(now, 'yyyy-mm-dd HH:MM:SS'));
    frt_v2_selected_expanded_switching(chunk, 358, chunk_tag);
end
