...
def select_best_actions(
    ...
    *,
    ...
    allow_reg_q: bool,
    prefer_voltage_survival: bool,
    prefer_full_frt: bool,
) -> dict[tuple[str, str], dict[str, Any]]:
    grouped: dict[tuple[str, str], list[dict[str, Any]]] = defaultdict(list)
    for row in rows:
        if str(row.get("mode")) not in allowed_modes:
            continue
        if not allow_reg_q and abs(f(row, "raw_m_reg_q")) > 1e-9:
            continue
        grouped[key_for(row)].append(row)
    selected: dict[tuple[str, str], dict[str, Any]] = {}
    for key, data in sorted(grouped.items()):
        if prefer_full_frt:
            full_rows = [row for row in data if truthy(row, "full_frt_pass") or truthy(row, "gbt_certifiable")]
            if full_rows:
                data = full_rows
        if prefer_voltage_survival:
            survival_rows = [row for row in data if truthy(row, "voltage_survival_pass")]
            if survival_rows:
                data = survival_rows
...
def main() -> None:
    ...
    parser.add_argument(
        "--allowed-modes",
        default="reg_sweep,joint_sweep",
        help="Comma-separated matrix modes eligible for teacher selection.",
    )
    parser.add_argument(
        "--allow-reg-q",
        action="store_true",
        help="Allow nonzero m_reg_q candidates. Required for full-FRT reactive-current teachers.",
    )
    parser.add_argument(
        "--no-prefer-voltage-survival",
        action="store_true",
        help="Do not first restrict selection to voltage-survival candidates when any exist.",
    )
    parser.add_argument(
        "--no-prefer-full-frt",
        action="store_true",
        help="Do not first restrict selection to full-FRT candidates when any exist.",
    )
    ...
    allowed_modes = parse_modes(args.allowed_modes)
    selected = select_best_actions(
        matrix_rows,
        allowed_modes=allowed_modes,
        allow_reg_q=args.allow_reg_q,
        prefer_voltage_survival=not args.no_prefer_voltage_survival,
        prefer_full_frt=not args.no_prefer_full_frt,
    )
    ...
    summary = {
        ...
        "allow_reg_q": bool(args.allow_reg_q),
        "prefer_voltage_survival": not args.no_prefer_voltage_survival,
        "prefer_full_frt": not args.no_prefer_full_frt,
    }
