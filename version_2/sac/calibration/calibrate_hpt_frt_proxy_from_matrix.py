...
``version_2/simulink/collectors/collect_hpt_v2_frt_calibration_matrix.m``.  The merged
...
def validate_envelope_columns(paths: list[Path], *, allow_legacy: bool = False) -> None:
    """Reject pre-envelope matrices unless explicitly requested."""
    ...
    if bad:
        ...
        raise RuntimeError(
            "Selected FRT matrix was produced before the timestep envelope gate.\n"
...
def envelope_metric_values(row: dict[str, Any]) -> dict[str, float | bool]:
    return {
