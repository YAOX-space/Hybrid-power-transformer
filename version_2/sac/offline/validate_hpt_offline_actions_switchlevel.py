...
def phase_pu_vector(phase_key: str, fault_pu: float) -> list[float]:
    phase = str(phase_key or "abc").strip().lower()
    if phase in {"abc", "balanced"}:
        return [fault_pu, fault_pu, fault_pu]
    if phase == "a":
        return [fault_pu, 1.0, 1.0]
    if phase == "b":
        return [1.0, fault_pu, 1.0]
    if phase == "c":
        return [1.0, 1.0, fault_pu]
    if phase == "ab":
        return [fault_pu, fault_pu, 1.0]
    if phase == "bc":
        return [1.0, fault_pu, fault_pu]
    if phase in {"ca", "ac"}:
        return [fault_pu, 1.0, fault_pu]
    raise ValueError(f"Unsupported phase_key: {phase_key}")
...
def matlab_struct(values: dict[str, float]) -> str:
    if not values:
        return "struct()"
    fields = []
...
def run_switch_case(
    ...
    *,
    ...
    phase_key = row.get("phase_key", "abc")
    phase_vector = phase_pu_vector(phase_key, fault_pu)
    ...
    statement = "; ".join(
        [
            ...
            f"hpt_compare_model_params={matlab_struct(COMMON_MODEL_PARAMS)}",
            f"hpt_compare_conventional_params={matlab_struct(STRONG_DQ_PARAMS)}",
            (
                f"hpt_compare_faults={{ {matlab_string(case_name)}, "
                f"{fault_pu:.12g}, {duration_s:.12g}, {matlab_vector(phase_vector)} }}"
            ),
            f"hpt_compare_fault_start={float(fault_start_s):.12g}",
