...
from version_2.sac.pretrain_hpt_actor_bc import collect_teacher_samples, train_actor_bc
...
class SplitBridgeHead(nn.Module):
    """Two physical output heads: regulating bridge and energy bridge."""

    def __init__(self, latent_dim: int, reg_dim: int = 2, energy_dim: int = 2):
        super().__init__()
        self.reg_head = nn.Linear(latent_dim, reg_dim)
        self.energy_head = nn.Linear(latent_dim, energy_dim)

    def forward(self, latent: Any) -> Any:
        return torch.cat((self.reg_head(latent), self.energy_head(latent)), dim=-1)


class SplitHeadSACActor(Actor):
    """SAC actor with separate final heads for reg and energy actions."""
...
class SplitHeadSACPolicy(SACPolicy):
    """SAC policy whose actor has independent reg/energy output heads."""
...
class EvalMetrics:
    ...
    fault_lv_band_violation_max_pu: float = 0.0
    fault_lv_band_violation_duration_s: float = 0.0
    envelope_violation_max_pu: float = 0.0
    envelope_violation_duration_s: float = 0.0
    recovery_violation_max_pu: float = 0.0
    recovery_violation_duration_s: float = 0.0
...
def latest_boundary_csv(directory: Path = CONTROL_DIR) -> Path:
    files = sorted(
        (
            p
            for p in directory.glob("control_comparison_*boundary*.csv")
            if "_summary" not in p.stem
        ),
        key=lambda p: p.stat().st_mtime,
    )
    if not files:
        raise FileNotFoundError(f"No boundary CSV found under {directory}")
    return files[-1]
...
def conventional_action_from_case(case: BaselineCase) -> np.ndarray:
    """Return controller command labels; measured bridge responses are not actions."""

    action = np.asarray(
        [
            f(case.row, "cmd_m_reg_d_fault_mean", f(case.row, "cmd_m_reg_d_mean", f(case.row, "raw_m_reg_d", 0.0))),
            f(case.row, "cmd_m_reg_q_fault_mean", f(case.row, "cmd_m_reg_q_mean", f(case.row, "raw_m_reg_q", 0.0))),
            f(case.row, "cmd_m_energy_d_fault_mean", f(case.row, "cmd_m_energy_d_mean", f(case.row, "raw_m_energy_d", 0.0))),
            f(case.row, "cmd_m_energy_q_fault_mean", f(case.row, "cmd_m_energy_q_mean", f(case.row, "raw_m_energy_q", 0.0))),
        ],
        dtype=np.float32,
    )
...
def select_baseline_cases(
    ...
    *,
    ...
    cases = [
        ...
        if str(row.get("mode", "")) in {"conventional_dq", "legacy_conventional"}
...
def baseline_metrics(case: BaselineCase, pass_column: str) -> EvalMetrics:
    ...
    metrics = EvalMetrics(
        ...
        fault_lv_band_violation_max_pu=finite(f(row, "fault_lv_band_violation_max_pu"), 0.0),
        fault_lv_band_violation_duration_s=finite(f(row, "fault_lv_band_violation_duration_s"), 0.0),
...
def score_metrics(metrics: EvalMetrics) -> float:
    ...
    score += 180.0 * max(0.0, finite(metrics.fault_lv_band_violation_max_pu)) ** 2
    score += 300.0 * max(0.0, finite(metrics.envelope_violation_max_pu)) ** 2
    score += 120.0 * max(0.0, finite(metrics.recovery_violation_max_pu)) ** 2
    score += 35.0 * max(0.0, finite(metrics.fault_lv_band_violation_duration_s))
    score += 60.0 * max(0.0, finite(metrics.envelope_violation_duration_s))
    score += 30.0 * max(0.0, finite(metrics.recovery_violation_duration_s))
...
def voltage_survival_pass(metrics: EvalMetrics) -> tuple[bool, str]:
    ...
    if metrics.fault_lv_band_violation_max_pu > 1e-3:
        reasons.append("timestep_fault_lv_band")
    if metrics.envelope_violation_max_pu > 1e-3:
        reasons.append("timestep_voltage_envelope")
    if metrics.recovery_violation_max_pu > 1e-3:
        reasons.append("timestep_recovery_envelope")
    ...
    *,
    ...
    if init_model is not None:
        ...
        policy: str | type[SACPolicy] = (
            SplitHeadSACPolicy if controller_heads == "split" else "MlpPolicy"
        )
        model = SAC(
            policy,
            vec,
    ...
    *,
        ...
        rows.append(
            {
                ...
                "fault_lv_band_violation": float(info.get("fault_band_violation_max_pu", 0.0)),
                "fault_lv_band_duration": float(info.get("fault_band_violation_duration_s", 0.0)),
    ...
    metrics = EvalMetrics(
        ...
        * env_config.v_ref_phase_rms
        ...
        fault_lv_band_violation_max_pu=float(np.max([r["fault_lv_band_violation"] for r in rows])),
        fault_lv_band_violation_duration_s=float(np.max([r["fault_lv_band_duration"] for r in rows])),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="split",
        help="Use separate regulating/energy actor heads or the old shared SAC head.",
    )
    parser.add_argument(
        "--bc-teacher-source",
            ...
            case_results.append(
                {
                    ...
                    "baseline_fault_lv_band_violation_max_pu": base.fault_lv_band_violation_max_pu,
                    "baseline_fault_lv_band_violation_duration_s": base.fault_lv_band_violation_duration_s,
                    "baseline_envelope_violation_max_pu": base.envelope_violation_max_pu,
                    "baseline_envelope_violation_duration_s": base.envelope_violation_duration_s,
                    "baseline_recovery_violation_max_pu": base.recovery_violation_max_pu,
                    "baseline_recovery_violation_duration_s": base.recovery_violation_duration_s,
                    ...
                    "sac_fault_lv_band_violation_max_pu": sac.fault_lv_band_violation_max_pu,
                    "sac_fault_lv_band_violation_duration_s": sac.fault_lv_band_violation_duration_s,
                    "sac_envelope_violation_max_pu": sac.envelope_violation_max_pu,
                    "sac_envelope_violation_duration_s": sac.envelope_violation_duration_s,
                    "sac_recovery_violation_max_pu": sac.recovery_violation_max_pu,
                    "sac_recovery_violation_duration_s": sac.recovery_violation_duration_s,
