...
def main() -> int:
    ...
    stage_commands: list[tuple[str, list[str], bool]] = [
        (
            "02_calibrate_proxy",
            py_module("version_2.sac.calibration.calibrate_hpt_frt_proxy_from_matrix", "--matrix-csv", matrix_csv),
            True,
        ),
        (
            "03_proxy_gap",
            py_module("version_2.sac.calibration.measure_hpt_frt_proxy_gap", "--matrix-csv", matrix_csv),
            False,
        ),
        (
            "04_reward_alignment",
            py_module("version_2.sac.calibration.measure_hpt_reward_alignment", "--matrix-csv", matrix_csv),
            False,
        ),
        (
            "05_energy_cmd_response",
            py_module(
                "version_2.sac.calibration.fit_hpt_energy_cmd_response",
                "--matrix-csv",
                matrix_csv,
                "--run-id",
                run_id,
