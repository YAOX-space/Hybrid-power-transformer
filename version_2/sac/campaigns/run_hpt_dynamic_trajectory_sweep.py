...
def metric_row(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "trajectory_envelope_violation_duration_s": f(traj, "envelope_violation_duration_s"),
        "trajectory_recovery_violation_max_pu": f(traj, "recovery_violation_max_pu"),
        "trajectory_recovery_violation_duration_s": f(traj, "recovery_violation_duration_s"),
        "trajectory_timestep_envelope_pass": truthy(traj.get("timestep_envelope_pass", "")),
        "fixed_envelope_violation_max_pu": f(fixed, "envelope_violation_max_pu"),
        "fixed_recovery_violation_max_pu": f(fixed, "recovery_violation_max_pu"),
        "baseline_envelope_violation_max_pu": f(baseline, "envelope_violation_max_pu"),
        "baseline_recovery_violation_max_pu": f(baseline, "recovery_violation_max_pu"),
...
def run_validation(args: argparse.Namespace, run_dir: Path, case: dict[str, Any]) -> dict[str, Any]:
    ...
    if args.fault_phase_pu:
        cmd += ["--fault-phase-pu", *[str(v) for v in args.fault_phase_pu]]
...
def build_cases(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for reg_pre in args.reg_pre_grid:
        for energy_d_pre in args.energy_d_pre_grid:
                            ...
                            for d_ramp_ms in args.d_ramp_ms_grid:
                                for q_ramp_ms in args.q_ramp_ms_grid:
                                    for down_ms in args.down_ms_grid:
                                        ...
                                        token = (
                                            f"{preset}_rp{safe_token(reg_pre)}_rd{safe_token(reg_d)}_"
                                            f"rq{safe_token(reg_q)}_ep{safe_token(energy_d_pre)}_"
                                        ...
                                        cases.append(
                                            {
                                                ...
                                                "q_ramp_ms": q_ramp_ms,
                                                "down_ms": down_ms,
                                                "base_action": [reg_pre, 0.0, energy_d_pre, 0.0],
...
def main() -> int:
    ...
    parser.add_argument("--reg-pre-grid", type=parse_grid, default=parse_grid("0"))
    parser.add_argument("--energy-d-pre-grid", type=parse_grid, default=parse_grid("0"))
