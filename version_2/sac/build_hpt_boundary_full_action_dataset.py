...
def latest_csv(directory: Path, pattern: str) -> Path:
    files = sorted(
        (p for p in Path(directory).glob(pattern) if "_summary" not in p.stem),
...
def canonical_row(
    ...
    *,
    ...
    out: dict[str, Any] = {
        "schema": "hpt-boundary-full-action-row-v1",
        ...
        "reason": s(row, "voltage_survival_reason", s(row, "full_frt_reason", "")),
    }
    return out
...
def main() -> int:
    ...
    rows: list[dict[str, Any]] = [
        canonical_row(row, row_source="conventional_boundary", role=role, pass_column=args.pass_column)
    ...
    manifest = {
        "schema": "hpt-boundary-full-action-dataset-v1",
        "run_id": run_id,
