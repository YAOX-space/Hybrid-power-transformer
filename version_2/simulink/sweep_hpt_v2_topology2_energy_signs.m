% sweep_hpt_v2_topology2_energy_signs
% Check topology2 energy-converter sign conventions under the dynamic
% programmable-source sag/swell test.  The regulating bridge stays disabled so
% the result isolates the energy bridge and parallel-coupled transformer path.
...
for f = 1:size(faults, 1)
    ...
    for s = 1:size(signCases, 1)
        rowCells{end+1} = run_case(M, faultName, faultPu, signCases(s, 1), ...
            signCases(s, 2), targetPhaseRms, stopTime, Ts, faultStart, faultClear); %#ok<SAGROW>
    end
end
close_system(M, 0);

rows = [rowCells{:}];
outDir = fullfile(rootDir, '..', '..', 'lab', 'results', 'hpt_v2_topology2_energy_sign_sweep');
if exist(outDir, 'dir') ~= 7
    mkdir(outDir);
end
stamp = datestr(now, 'yyyymmdd_HHMMSS');
outMat = fullfile(outDir, ['topology2_energy_sign_sweep_' stamp '.mat']);
outCsv = fullfile(outDir, ['topology2_energy_sign_sweep_' stamp '.csv']);
save(outMat, 'rows', 'targetPhaseRms', 'nominalGridVoltage', ...
    'faultStart', 'faultClear', 'stopTime', 'signCases');
writetable(struct2table(rows), outCsv);

fprintf('Topology2 energy sign sweep complete.\n');
fprintf('%-10s %8s %8s %10s %10s %10s %9s %9s\n', ...
    'fault', 'ctrl', 'pol', 'LV_fault', 'LV_recov', 'LV_peak', 'VdcMin', 'VdcMean');
for i = 1:numel(rows)
    fprintf('%-10s %8.0f %8.0f %10.3f %10.3f %10.3f %9.3f %9.3f\n', ...
        rows(i).fault, rows(i).control_sign, rows(i).polarity, rows(i).lv_fault_rms_mean, ...
        rows(i).lv_recovery_rms_mean, rows(i).lv_peak_rms, rows(i).vdc_min, rows(i).vdc_mean);
end
fprintf('Saved MAT: %s\n', outMat);
fprintf('Saved CSV: %s\n', outCsv);

function row = run_case(M, faultName, faultPu, controlSign, polarity, ...
    ...
    in = in.setVariable('hpt_energy_control_sign', controlSign, 'Workspace', M);
    in = in.setVariable('hpt_energy_bridge_polarity', polarity, 'Workspace', M);
    out = sim(in);
