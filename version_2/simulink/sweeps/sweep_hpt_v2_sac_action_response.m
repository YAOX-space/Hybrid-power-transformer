% sweep_hpt_v2_sac_action_response
% Calibrate the averaged HPT SAC proxy against the physical switch-level
% topology1/topology2 Simulink models.
%
% The sweep runs the HPTSACController in fixed-action mode:
%   hpt_sac_policy_mode = -1
%   hpt_sac_enable = 1
%   hpt_sac_energy_enable = 0
%
% This lets the physical regulating converter see known [m_reg_d,m_reg_q]
% commands while the energy bridge remains on the conventional DC-link loop.
...
mRegDValues = [-0.80, -0.40, 0.0, 0.40, 0.80];
mRegQValues = 0.0;
mEnergyD = 0.0;
mEnergyQ = 0.0;
stopTime = 0.08;
...
for c = 1:size(cases, 1)
    ...
    for gv = 1:numel(gridVoltages)
        for id = 1:numel(mRegDValues)
            for iq = 1:numel(mRegQValues)
                rowCells{end+1} = run_fixed_action_case( ...
                    M, topology, gridVoltages(gv), targetPhaseRms, ...
                    mRegDValues(id), mRegQValues(iq), mEnergyD, mEnergyQ, ...
...
outDir = fullfile(rootDir, '..', '..', 'lab', 'results', 'hpt_v2_sac_proxy_sweep');
if exist(outDir, 'dir') ~= 7
    mkdir(outDir);
end
stamp = datestr(now, 'yyyymmdd_HHMMSS');
outMat = fullfile(outDir, ['hpt_v2_sac_proxy_sweep_' stamp '.mat']);
outCsv = fullfile(outDir, ['hpt_v2_sac_proxy_sweep_' stamp '.csv']);
save(outMat, 'rows', 'targetPhaseRms', 'gridVoltages', 'mRegDValues', ...
    'mRegQValues', 'mEnergyD', 'mEnergyQ', 'stopTime', 'settleStart');
writetable(struct2table(rows), outCsv);

fprintf('HPT SAC fixed-action switch-level sweep complete.\n');
fprintf('Target phase RMS: %.1f V, settle window: t > %.3f s\n', ...
    targetPhaseRms, settleStart);
fprintf('%-10s %8s %8s %8s %10s %11s %9s %9s %9s\n', ...
    'topology', 'grid', 'cmdD', 'actD', 'LV_RMS', 'unbalance', ...
    'VdcMean', 'VdcMin', 'obsN');
for i = 1:numel(rows)
    fprintf('%-10s %8.0f %8.3f %8.3f %10.3f %11.3f %9.3f %9.3f %9.0f\n', ...
        rows(i).topology, rows(i).grid_V, rows(i).cmd_m_reg_d, ...
        rows(i).reg_d_mean, rows(i).lv_rms_mean, rows(i).lv_unbalance, ...
        rows(i).vdc_mean, rows(i).vdc_min, rows(i).obs_dim);
end
fprintf('Saved MAT: %s\n', outMat);
fprintf('Saved CSV: %s\n', outCsv);

function row = run_fixed_action_case(M, topology, gridVoltage, targetPhaseRms, ...
    mRegD, mRegQ, mEnergyD, mEnergyQ, stopTime, settleStart, Ts)
    ...
    row.action_raw_source = "hpt_sac_fixed_*";
