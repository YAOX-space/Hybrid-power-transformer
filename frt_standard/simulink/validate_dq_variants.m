...
modes=[4 5 6]; mlab={'dq-legacy','dq-Song(no-series)','dq-Jia(Vdc-derate)'};
...
R=struct();
fprintf('\nLVRT %d scenarios x modes [4 5 6]\n',N);
for i=1:N
...
fprintf('\nHVRT %d scenarios x modes [4 5 6]\n',Nh);
for i=1:Nh
...
% ================= summary =================
save('../results/dq_variants_compare.mat','R','mlab');
fid=fopen('../results/dq_variants_compare.txt','w');
...
emit(sprintf('%-22s LVRT %5.1f%%  HVRT %5.1f%%  ALL-32 %5.1f%%   (from prior subset run)\n','SAC 4-expert mode-12',45.8,75.0,53.1));
fclose(fid); fprintf('\nsaved ../results/dq_variants_compare.{mat,txt}\n');
end
